...
async function handleAutoReactCommand(message, client) {
  if (message.content.trim().startsWith("&ar")) {
    ...
    if (args.length !== 3) {
      message.reply("用法：&ar <emojiID> <channelID>");
      return true;
    }
    ...
    // 驗證 emoji 格式
    if (!/^\d+$/.test(emojiId)) {
      message.reply("Emoji ID 格式錯誤，請提供數字 ID");
      return true;
    }
  ...
  if (message.content.trim().startsWith("&ar remove")) {
    const args = message.content.trim().split(" ");
    if (args.length !== 3) {
      message.reply("用法：&ar remove <channelID>");
      return true;
    }
...
async function handleOdogCommand(message, client) {
  // 處理自動回應指令
  if (await handleAutoReactCommand(message, client)) return true;
